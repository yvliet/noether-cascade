...
const buildOptions = {
  entryPoints: [path.join(__dirname, 'CascadeExtension.tsx')],
  bundle: true,
...
async function run() {
  if (isWatch) {
    ...
    await ctx.watch();
    console.log('[Watch] Watching for changes in Cascade...');
  } else {
    await esbuild.build(buildOptions);
    const stats = fs.statSync(path.join(__dirname, 'dist/main.js'));
    console.log(`[Build] Generated dist/main.js for Cascade (${(stats.size / 1024).toFixed(1)} KB)`);
  }
}
